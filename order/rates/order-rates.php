 ...
 */
if (!class_exists('EnSaiaOrderRates')) {

    class EnSaiaOrderRates
    {
        ...
        public function en_order_before_calculate_totals($and_taxes, $order)
        {
            ...
            if (isset($woocommerce->cart)) {
                    ...
                    foreach ($shipping as $item_id => $per_shipment) {
                        ...
                        switch ($method_id) {
                            case 'saia':
                                ...
                                if (empty($errors)) {
                                    $shipping_class = new SAIA_Freight_Shipping_Class();
                ...
                if ($order_recreated) {
                    ...
                    $saia_freight_curl_obj = new SAIA_Curl_Request();
                    $saia_freight_curl_obj->saia_get_curl_response(SAIA_FDO_HITTING_URL, $request_data);
                }
            }
        }
    }

    new EnSaiaOrderRates();
}