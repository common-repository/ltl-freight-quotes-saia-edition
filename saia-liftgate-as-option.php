...
if(!class_exists("Saia_Liftgate_As_Option"))
{
    class Saia_Liftgate_As_Option{
        
        /**
         * set flag for lift gate as option
         * @var string type 
         */
        public $saia_as_option;
        ...
        {
            $this->saia_as_option = get_option("saia_quotes_liftgate_delivery_as_option");
         ...
         * @return array string
         */
        public function saia_update_carrier_service( $post_data )
        {
            if(isset($this->saia_as_option) && ($this->saia_as_option == "yes"))
            {
         ...
         * @return array type
         */
        public function update_parse_saia_output( $surcharges )
        {         
            $surcharge_amount = array();      
            foreach ($surcharges as $key => $surcharge) {
                if(isset($surcharge->Code)){
                    $surcharge_amount[$surcharge->Code] = (isset($surcharge->Charges)) ? $surcharge->Charges / 100 : 0;
                }
            }
         ...
         * @return array type
         */
        public function update_rate_whn_as_option_saia( $rate )
        {
            ...
            if(isset($rate) && (!empty($rate))){
                ...
                $rate = apply_filters("en_woo_addons_web_quotes" , $rate , en_woo_plugin_saia_quotes);
                
                $label_sufex = (isset($rate['label_sufex'])) ? $rate['label_sufex'] : array();  
                $label_sufex = $this->label_R_saia($label_sufex);
                $rate['label_sufex'] = $label_sufex;
                
                if(isset($this->saia_as_option,$rate['grandTotalWdoutLiftGate']) && 
                        ($this->saia_as_option == "yes") && ($rate['grandTotalWdoutLiftGate'] > 0))
                {
         ...
         * @return aray type
         */
        public function filter_label_sufex_array_saia($result)
        {
            $this->label_sfx_arr = [];
         ...
         * @return array type
         */
        public function label_R_saia($label_sufex)
        {
