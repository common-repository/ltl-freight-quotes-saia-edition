...
/**
 * SAIA WooComerce | Shipping Calculation Method
...
/**
 * SAIA WooComerce | SAIA Freight Initialize
 */
function saia_logistics_init()
{
    if (!class_exists('SAIA_Freight_Shipping_Class')) {

        /**
         * SAIA WooComerce | Shipping Calculation Class
         */
        class SAIA_Freight_Shipping_Class extends WC_Shipping_Method
        {

            public $forceAllowShipMethod = array();
            public $getPkgObj;
            public $Saia_Liftgate_As_Option;
            ...
            public function __construct($instance_id = 0)
            {
                ...
                $this->method_title = __('SAIA Freight');
                $this->method_description = __('Shipping rates from SAIA Freight.');
                $this->supports = array(
                    'shipping-zones',
                    'instance-settings',
                ...
                $this->enabled = "yes";
                $this->title = 'LTL Freight Quotes - SAIA Edition';
                $this->init();
                $this->Saia_Liftgate_As_Option = new Saia_Liftgate_As_Option();

//              Minify Me
                $this->min_prices = array();
            }
...
//              return the rates
            ...
            /**
             * Enable WooCommerce Shipping For SAIA
             */
            function init_form_fields()
            {
                $this->instance_form_fields = array(
                    'enabled' => array(
                        'title' => __('Enable / Disable', 'saia'),
                        'type' => 'checkbox',
                        'label' => __('Enable This Shipping Service', 'saia'),
                        'default' => 'no',
                        'id' => 'saia_enable_disable_shipping'
                    )
                );
            }
            ...
            {
                if (!empty($this->getPkgObj->ValidShipmentsArrSaia) && (!in_array("ltl_freight", $this->getPkgObj->ValidShipmentsArrSaia))) {
            ...
            /**
             * Calculate Shipping Rates For SAIA
            ...
            public function calculate_shipping($package = array(), $eniture_admin_order_action = false)
            {
                ...
                $this->package_plugin = get_option('saia_quotes_packages_quotes_package');
...
//              Eniture debug mood
                ...
                if (isset($coupn) && !empty($coupn)) {
                    $free_shipping = $this->saia_shipping_coupon_rate($coupn);
                    if ($free_shipping == 'y')
                        return [];
                }
                $saia_woo_obj = new SAIA_Woo_Update_Changes();
                $freight_zipcode = "";
                (strlen(WC()->customer->get_shipping_postcode()) > 0) ? $freight_zipcode = WC()->customer->get_shipping_postcode() : $freight_zipcode = $saia_woo_obj->saia_postcode();

                $obj = new SAIA_Shipping_Get_Package();

                $this->getPkgObj = $obj;

                $saia_res_inst = new SAIA_Get_Shipping_Quotes();

                $this->web_service_inst = $saia_res_inst;

                $this->saia_ltl_shipping_quote_settings();

                if (!empty($this->web_service_inst->quote_settings['handling_fee']) && $this->web_service_inst->quote_settings['handling_fee'] == "-100%") {
                    $rates = array(
                        'id' => $this->id . ':' . 'free',
                        'label' => 'Free Shipping',
                        'cost' => 0,
                ...
                // Eniture debug mood
                do_action("eniture_debug_mood", "Quote Settings", $this->web_service_inst->quote_settings);

                $saia_package = $obj->group_saia_shipment($package, $saia_res_inst, $freight_zipcode);
                $handlng_fee = get_option('saia_handling_fee');
                $quotes = array();
                $rate = array();
                ...
                if (isset($saia_package) && !empty($saia_package)) {
                    $small_products = [];
                    $ltl_products = [];
                    $thirdparty_postal_code = get_option('wc_settings_saia_accountnbr_postal_code');
                    foreach ($saia_package as $locId => $sPackage) {
                        if (array_key_exists('saia', $sPackage)) {
                            $ltl_products[] = $sPackage;
                            $web_service_arr = $saia_res_inst->saia_shipping_array($sPackage, $this->package_plugin);
                            $response = $saia_res_inst->saia_get_web_quotes($web_service_arr);
                            if (empty($response)) {
                                return [];
                            }
                            $quotes[] = $response;
                ...
                if (count($quotes) > 1 || $smpkgCost > 0 || !empty($virtual_rate)) {
                    ...
                    (isset($small_quotes) && count($small_quotes) > 0) ? $this->minPrices['SAIA_LIFT'] = $small_quotes : "";
                    (isset($small_quotes) && count($small_quotes) > 0) ? $this->minPrices['SAIA_NOTLIFT'] = $small_quotes : "";
                    (isset($small_quotes) && count($small_quotes) > 0) ? $this->minPrices['SAIA_HAT'] = $small_quotes : "";
                    // Virtual products
                    if (!empty($virtual_rate)) {
                        ...
                        $this->minPrices['SAIA_LIFT'] = isset($this->minPrices['SAIA_LIFT']) && !empty($this->minPrices['SAIA_LIFT']) ? array_merge($this->minPrices['SAIA_LIFT'], $virtual_meta_rate) : $virtual_meta_rate;
                        $this->minPrices['SAIA_NOTLIFT'] = isset($this->minPrices['SAIA_NOTLIFT']) && !empty($this->minPrices['SAIA_NOTLIFT']) ? array_merge($this->minPrices['SAIA_NOTLIFT'], $virtual_meta_rate) : $virtual_meta_rate;
                        ...
                        if ($this->quote_settings['HAT_status'] == 'yes') {
                            $this->minPrices['SAIA_HAT'] = isset($this->minPrices['SAIA_HAT']) && !empty($this->minPrices['SAIA_HAT']) ? array_merge($this->minPrices['SAIA_HAT'], $virtual_meta_rate) : $virtual_meta_rate;
                        }
                    }
                    ...
                    foreach ($quotes as $key => $quote) {
                        if (!empty($quote)) {
                            ...
                            if (isset($quote['hold_at_terminal_quotes'])) {
                                ...
                                $this->minPrices['SAIA_HAT'][$key] = $hold_at_terminal_quotes;

                                // product level markup
                                if(!empty($hold_at_terminal_quotes['product_level_markup']) && !empty($hold_at_terminal_quotes['cost'])){
                                    $hold_at_terminal_quotes['cost'] = $this->add_handling_fee($hold_at_terminal_quotes['cost'], $hold_at_terminal_quotes['product_level_markup']);
                                }

                                // origin level markup
                                if(!empty($hold_at_terminal_quotes['origin_markup']) && !empty($hold_at_terminal_quotes['cost'])){
                                    $hold_at_terminal_quotes['cost'] = $this->add_handling_fee($hold_at_terminal_quotes['cost'], $hold_at_terminal_quotes['origin_markup']);
                                }

                                $hold_at_terminal_quotes['meta_data']['en_fdo_meta_data']['rate']['cost'] = $hold_at_terminal_quotes['cost'];

                                // FDO
                                $this->en_fdo_meta_data['SAIA_HAT'][$key] = (isset($hold_at_terminal_quotes['meta_data']['en_fdo_meta_data'])) ? $hold_at_terminal_quotes['meta_data']['en_fdo_meta_data'] : [];
                            ...
                            $this->minPrices['SAIA_LIFT'][$key] = $rates;

                            // FDO
                            $this->en_fdo_meta_data['SAIA_LIFT'][$key] = (isset($rates['meta_data']['en_fdo_meta_data'])) ? $rates['meta_data']['en_fdo_meta_data'] : [];
...
//                          Offer lift gate delivery as an option is enabled
                            if (isset($this->quote_settings['liftgate_delivery_option']) &&
                                ...
                                $this->minPrices['SAIA_NOTLIFT'][$key] = $s_rates;

                                $this->en_fdo_meta_data['SAIA_NOTLIFT'][$key] = (isset($s_rates['meta_data']['en_fdo_meta_data'])) ? $s_rates['meta_data']['en_fdo_meta_data'] : [];
                                ...
                                $this->minPrices['SAIA_NOTLIFT'][$key]['cost'] = $this->add_handling_fee($s_cost, $handling_fee);
                                $this->minPrices['SAIA_NOTLIFT'][$key]['meta_data']['en_fdo_meta_data']['rate']['cost'] = $this->en_fdo_meta_data['SAIA_NOTLIFT'][$key]['rate']['cost'] = $this->minPrices['SAIA_NOTLIFT'][$key]['cost'];
                            }
                            ...
                            $this->minPrices['SAIA_LIFT'][$key]['cost'] = $this->add_handling_fee($_cost, $handling_fee);
                            $this->minPrices['SAIA_LIFT'][$key]['meta_data']['en_fdo_meta_data']['rate']['cost'] = $this->en_fdo_meta_data['SAIA_LIFT'][$key]['rate']['cost'] = $this->minPrices['SAIA_LIFT'][$key]['cost'];
                            $shipment_numbers++;
                        }
                    }
                    ...
                    ($s_multi_cost > 0) ? $rate[] = $this->arrange_multiship_freight(($s_multi_cost + $smpkgCost), 'SAIA_NOTLIFT', $s_label, $s_append_label) : "";
                    ($multi_cost > 0 || $smpkgCost > 0) ? $rate[] = $this->arrange_multiship_freight(($multi_cost + $smpkgCost), 'SAIA_LIFT', $_label, $append_label) : "";
                    ($hold_at_terminal_fee > 0) ? $rate[] = $this->arrange_multiship_freight(($hold_at_terminal_fee + $smpkgCost), 'SAIA_HAT', $hat_label, $append_hat_label) : "";
                    $rates = $rate;
...
//                      Offer lift gate delivery as an option is enabled
                        ...
                        if (is_array($rates) && (!empty($rates))) {
                            foreach ($rates as $key => $quote) {
                                ...
                                if (isset($quote['hat_append_label'])) {
                                    $rates[$key]['cost'] = $rates[$key]['meta_data']['en_fdo_meta_data']['rate']['cost'] = $_cost;
                                }else{
                                    (isset($rates[$key]['cost'])) ? $rates[$key]['cost'] = $rates[$key]['meta_data']['en_fdo_meta_data']['rate']['cost'] = $this->add_handling_fee($_cost, $handling_fee) : "";
                                }
                    ...
                    $this->InstorPickupLocalDelivery = $saia_res_inst->saia_ltl_return_local_delivery_store_pickup();
                ...
                if (isset($rates)) {
                    $rates = $this->saia_add_rate_arr($rates);
                }
            ...
            function arrange_multiship_freight($cost, $id, $label_sufex, $append_label)
            {
                ...
                ($id == 'SAIA_HAT') ? $multiship['hat_append_label'] = $append_label : $multiship['append_label'] = $append_label;
             ...
             */
            public function saia_add_rate_arr($add_rate_arr)
            {
            ...
            public function set_label_in_quote($rate)
            {
                ...
                $rate_label = ($this->quote_settings['shipment'] == "single_shipment") ? $this->saia_label_as() : 'Freight';

                $rate_label .= $this->saia_filter_from_label_sufex($label_sufex);
                ...
                if ($this->quote_settings['delivery_estimates'] == 'yes' && $this->quote_settings['shipment'] == "single_shipment") {
                    $estimtdDays = (isset($rate['transit_time'])) ? $rate['transit_time'] : '';
                    $rate_label .= ' (Intransit days: ' . $estimtdDays . ')';
                }

                return $rate_label;
            }

            public function saia_filter_from_label_sufex($label_sufex)
            {
                $append_label = "";
                $rad_status = true;
             ...
             * @return string
             */
            function saia_shipping_coupon_rate($coupon)
            {
             ...
             * @global $wpdb $wpdb
             */
            function saia_ltl_shipping_quote_settings()
            {
                $this->web_service_inst->quote_settings['label_as'] = get_option('saia_label_as');
                $this->web_service_inst->quote_settings['handling_fee'] = get_option('saia_handling_fee');
                $this->web_service_inst->quote_settings['liftgate_delivery'] = get_option('saia_liftgate');
                $this->web_service_inst->quote_settings['liftgate_delivery_option'] = get_option('saia_quotes_liftgate_delivery_as_option');
                $this->web_service_inst->quote_settings['residential_delivery'] = get_option('saia_residential');
                $this->web_service_inst->quote_settings['liftgate_resid_delivery'] = get_option('en_woo_addons_liftgate_with_auto_residential');
                $this->web_service_inst->quote_settings['delivery_estimates'] = get_option('saia_delivey_estimate');
                $this->web_service_inst->quote_settings['HAT_status'] = get_option('saia_ltl_hold_at_terminal_checkbox_status');
                $this->web_service_inst->quote_settings['HAT_fee'] = get_option('saia_ltl_hold_at_terminal_fee');
                $this->web_service_inst->quote_settings['handling_weight'] = get_option('saia_freight_settings_handling_weight');
                $this->web_service_inst->quote_settings['maximum_handling_weight'] = get_option('saia_freight_freight_maximum_handling_weight');
            }
